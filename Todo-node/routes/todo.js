    ...
    function verifyAuthToken(req) {
      ...
      if(authorizationHeader){
        ...
        return {status : false,message: "Token not available"}
      }
   }
    ...
    router.post('/addTask', async (req, res) => {
      try {
        ...
        if (verifyToken.status) {
          ...
          const newTask = await ToDo.create({ taskName, userId });
          if (newTask) {
            return res.status(201).json({
              status: "success",
              message: 'Task added successfully',
              data: newTask, // Include task data in the response if needed
            });
          } else {
          ...
          return res.status(401).json({ status: "failure", message: verifyToken.message });
        }
      } catch (error) {
        console.error(error);
  ...
  router.get('/todos', async (req, res) => {
    console.log("data",req.query.status);
    try {
      const verifyToken = verifyAuthToken(req);
      if(verifyToken.status){
        const userId = req.headers['userid'];
        ...
        var completedCounts;
        if(req.query.activeTab=="All"){
          todos = await ToDo.findAll({ where: { deleteFlag: 0, userId: userId } });
        }else{
          if(req.query.activeTab=="In-progress"){
            status=0;
          }else{
            status=1;
          }
